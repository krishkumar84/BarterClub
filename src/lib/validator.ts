...
export const productSchema = z.object({
  ...
  availableQty: z.number().min(1, { message: 'Available quantity must be at least 1' }),
  deliveryTime: z.number().min(1, { message: 'Delivery time must be at least 1' }),
  categoryId: z.string().optional(),
  price: z.number().nonnegative({ message: "Price must be a positive number" }),
  gst: z.number().nonnegative({ message: "GST must be a non-negative number" }).optional(),
